...
const TabItem: FC<Props> = ({ github, image, label, url }) => {

    const [hoveredImg, setHoveredImg] = useState(false);

    return (
        <motion.div
            onClick={() => window.innerWidth <= 768 && setHoveredImg(e => !e)}
            onHoverStart={() => window.innerWidth > 768 && setHoveredImg(e => true)}
            onHoverEnd={() => window.innerWidth > 768 && setHoveredImg(e => false)}
        ...
        >
            <motion.div
                ...
                <p className='font-bold text-md text-center'>{label}</p>
